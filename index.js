...
function generateMessage() {
  let queIndex = Math.floor(Math.random() * questions.length);
  let ques_display = questions[queIndex];
  let ans = answers[queIndex];
  let html = ``;
  html += `<h2>${ques_display}</h2>`;
  html += `<p>${ans}</p>`;
  document.getElementById("output").style.display = "block";
  document.getElementById("output").innerHTML = html;
}

document.getElementById("myBtn").addEventListener("click", generateMessage);
